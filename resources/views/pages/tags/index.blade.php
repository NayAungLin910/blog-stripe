    ...
    <section class="container pt-16 mx-auto mb-32">
        ...
        <div class="grid grid-cols-4 gap-6" data-aos="fade-up" data-aos-offset="100" data-aos-duration="500">
            ...
            <a href="{{ route('tags.show', $tag) }}" class="relative col-span-1 cursor-pointer hover:shadow">
                <img class="w-full h-full transition-all obeject-cover opacity-80 duration-250 hover:opacity-100"
                    src="{{ $tag->publicImageLink() }}" alt="{{ $tag->name() }}">
                <h2
                    class="absolute inset-x-auto w-full py-2 font-serif text-5xl font-bold text-center text-white bg-gray-600 bg-opacity-50 text-shadow top-2/4">
                    {{ $tag->name() }}
                </h2>
            </a>
            @endforeach
